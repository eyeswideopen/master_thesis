...
for imageTuple in tqdm(sliceTupelList):
    ...
    #all tumors removed -> only liver segmentation!
    if len(colors) == 3:
        data[data == 150] = 1
        data[data == 255] = 1
        im = Image.fromarray(data)
    ...
    # rotate by 180 degrees if neccessary 
    if rotate:
        rotatedImages += 1
        data = np.rot90(data,2)

    im.save(os.path.join(outputFolder, imageTuple[0].split('_')[0] + '_' + str(sliceIndex) + '_label.png'), "png")
    ...
    im.save(os.path.join(outputFolder, imageTuple[1].split('_')[0] + '_' + str(sliceIndex) + '_color.png'), "png")
    sliceIndex += 1
print("Data preprocessing finished!")
print("Images rotated: " + str(rotatedImages))
